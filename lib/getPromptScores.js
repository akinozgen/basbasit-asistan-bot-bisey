export function getPromptScore(prompt, responseTexts) {
    ...
    const promptWords = prompt.trim().replace(/[^\w\sğüşıöçĞÜŞİÖÇ]/gi, '').toLocaleLowerCase().split(/\s+/);

    for (const [key, responseText] of Object.entries(responseTexts)) {
        ...
        for (const possiblePrompt of possiblePrompts) {
            const possiblePromptWords = possiblePrompt.trim().replace(/[^\w\sğüşıöçĞÜŞİÖÇ]/gi, '').toLocaleLowerCase().split(/\s+/);
